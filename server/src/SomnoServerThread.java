...
public class SomnoServerThread extends Thread implements SomnoProtocol {
    ...
    private String pwd;
    private boolean pwdIsValid = true;
     ...
     * The op specifies how the message should be formatted
     *
     * @param msg the message to send out
     * @param op the operation used to format the message
     */
    public synchronized void update(String msg, operation op) {
    ...
    //something is definitely wrong with the nickname system and its documentation
    /**
    ...
    public void run() {
        ...
        try {
            ...
            //requires the "/pwd password" protocol format to work properly
            ...
            if (!pwd.equals("")) {
                ...
                if(!(split_cmd[1].equals(pwd))) {
                    ...
                    closeConnections(receive, send);
                    pwdIsValid = false;
                    //bug happens because the rest of the code till runs
                }

            }
            if (pwdIsValid) {
                nickname = getValidNickname(receive.readLine(), 1); //gets the nickname
                update(nickname, operation.NEW_CONNECTION);
            ...
            System.err.println("IOException caught. Client may have closed unexpectedly");
            //call signoff message stuff
            update(nickname, operation.DISCONNECTION);
        }
